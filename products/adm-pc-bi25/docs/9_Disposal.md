# **7\. Disposal**

As per the [Safety Instructions](https://documentation.advantics.fr/adm-pc-bi25/1_Safety_instructions/), if safe operation of the instrument can no longer be assumed, it must be removed from the factory, laboratory and secured against unintentional use. 

Safe operation can be no longer assumed if: 

* The module, power cables or any cables show visible damage  
* The module no longer functions  
* The module appears to be very dirty, or is covered in dust/debris etc.   
* After lengthy periods of storage under unfavorable conditions, with the exposed metal parts of the converter showing visible corrosion or deterioration  
* After exposure to unusual transport stresses (dislodged connectors, bent pins, missing SMD components, etc.)

