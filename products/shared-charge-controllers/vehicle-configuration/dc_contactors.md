...
<figcaption>Example</figcaption>
    ...
    dc_contactors_ios_delay = 0

Delay, in seconds, between the moment the controller commands the contactors through its IOs and
the moment it considers them closed. It only applies when `dc_contactors_use_ios` is true and
`dc_contactors_ios_has_feedback` is false: with no feedback wired, the controller has no way to
observe the actual transition, so it waits this long instead.
