...
This page is the starting point for integrating the ADM-CS-MEVC into a vehicle. It walks
...
!!! info "Who this is for"
    Vehicle integrators of an **MCS** (Megawatt Charging System) vehicle — typically a heavy-duty
    truck, a vessel, and etc. It assumes you are responsible for the VCU/BMS side and the wiring harness. For
...
The ADM-CS-MEVC owns the MCS inlet and the ISO 15118-20 charging protocol over 10BASE-T1S, and
exposes the whole charge process to the rest of the vehicle as CAN bus interface. It cannot
function on its own: the vehicle must supply a few measurements and a small amount of
decision-making.
...
|---|---|---|---|
| **DC contactors** ([§5](#5-dc-contactor-control)) | H-bridge outputs, plus feedback inputs | [DC_Control.Close_Contactors](vehicle-can-interfaces/can_v2.md#DC_Control-Close_Contactors) and [DC_Status2.Contactors_Closed](vehicle-can-interfaces/can_v2.md#DC_Status2-Contactors_Closed) | [dc_contactors_use_ios](configuration/dc_contactors.md#dc_contactors_use_ios), [dc_contactors_ios_has_feedback](configuration/dc_contactors.md#dc_contactors_ios_has_feedback) |
| **Inlet & battery voltage, current** ([§6](#6-voltage-and-current-measurement)) | A supported CAN sensor the controller reads itself | [DC_Status1](vehicle-can-interfaces/can_v2.md#dc_status1) / [DC_Status2](vehicle-can-interfaces/can_v2.md#dc_status2) from your BMS | [use_can_sensor](configuration/can_sensor.md) |
| **Inlet lock** ([§7](#7-inlet-lock)) | Lock motor drive and resistive feedback | Hardware only | [no_inlet_lock](configuration/inlet_lock.md#no_inlet_lock) disables it for bench tests |
| **Digital inputs** ([§8](#8-digital-inputs-outputs-and-leds)) | `Stop`, `Emergency_Stop`, `Sleep`, `Monitor` on `dig_inN` | Reported in [ADM_CS_EVCC_Inputs](vehicle-can-interfaces/can_v2.md#adm_cs_evcc_inputs) | `[hardware] dig_inN` |
| **Digital outputs, LEDs** ([§8](#8-digital-inputs-outputs-and-leds)) | `Plugged_In`, `Contactor_Enable` on `dig_outN` / `ledN` | Driven by [ADM_CS_EVCC_MEVC_Outputs](vehicle-can-interfaces/can_v2.md#adm_cs_evcc_mevc_outputs) | `dig_outN = CAN_Controlled` |
| **Inlet temperature** ([§9](#9-temperature-monitoring)) | PT1000 inputs | Reported in [ADM_CS_EVCC_Inputs](vehicle-can-interfaces/can_v2.md#adm_cs_evcc_inputs) | Temperature channel configuration |
| **Stopping a charge** ([§11](#11-stopping-a-charge)) | `Stop` input | Over the generic interface | `[hardware] dig_inN = Stop` |
| **Diagnostics** ([§10](#10-diagnostics)) | — | [EVCC_MEVC_Diagnostic_Status](vehicle-can-interfaces/can_v2.md#evcc_mevc_diagnostic_status) | Always on |
| **Your BMS dialect** ([§3.3](#33-if-your-vehicle-speaks-j1939)) | — | Generic interface, or **SAE J1939** through the bridge | [type](configuration/generalities.md#type), `enable_j1939_bridge` | -->

<!-- The rest of the page is one section per row, in that order. -->

{{ figure('assets/integration-overview.svg', 'What the ADM-CS-MEVC owns, what you provide, and the interfaces that cross between') }}
...
| Inlet harness: TRXP/TRXN, CE, ID, lock motor, PT1000 sensors | Fully driven by the communication stack — you only wire it | [MCS interface](interfaces.md#mcs-interface) |
| **Inlet** voltage, battery voltage and charge current | Required by the standards, and by the controller to run precharge and to unlock safely | [§6](#6-voltage-and-current-measurement) |
| A way to open and close the DC contactors | To expose the battery to the charger | [§5](#5-dc-contactor-control) |
...
| [`energy_capacity`](configuration/generalities.md#energy_capacity) | Required to derive energy requests; mandatory in ISO15118-20 | [Generalities](configuration/generalities.md) |
...
## 2. The MCS inlet interface

Unlike CCS, MCS carries high-level communication over **10BASE-T1S** rather than PLC on the Control
Pilot, and it uses **CE** (Charge Enable) and **ID** lines rather than CP and PP.

| Line | Role |
|---|---|
| `TRXP` / `TRXN` | 10BASE-T1S high-level communication (MCS PHY1 / PHY2) |
| `CE` | Charge Enable — the equivalent of CCS's CP states, and the signal whose loss forces a hardware disconnect |
| `ID` | Connector identification |
| Inlet lock | Motorised lock, see [§7](#7-inlet-lock) |
| PT1000 inputs | Inlet temperature, see [§9](#9-temperature-monitoring) |

CE and ID are sampled continuously and can be disturbed by electrical noise, so the `[ccs]` section
exposes filtering:

| Concern | Entry |
|---|---|
| Median filter on CE/ID | [mcs_ce_id_use_median_filter](configuration/mcs.md#mcs_ce_id_use_median_filter), [mcs_ce_id_filter_buffer_size](configuration/mcs.md#mcs_ce_id_filter_buffer_size) |
| Debouncing | [mcs_ce_id_use_debouncer](configuration/mcs.md#mcs_ce_id_use_debouncer), [mcs_ce_id_debouncer_count](configuration/mcs.md#mcs_ce_id_debouncer_count) |

!!! warning "Filtering costs latency, and latency breaks the sequence"
    Both filters are **off** by default and should stay off unless you have a measured noise
    problem. The safety-critical CE/ID functions are handled by dedicated hardware logic, so the
    software sampling rate is deliberately low; a large `mcs_ce_id_filter_buffer_size` or a high
    `mcs_ce_id_debouncer_count` — or both filters at once — delays state-change detection enough
    for the MCS state machine to miss time-sensitive transitions, which surfaces as sequence
    timeouts and sessions ending for no visible reason. Tune one at a time, conservatively.

## 3. The vehicle CAN bus — your main integration surface
...
|---|---|
| Bit rate | 500 kbaud by default; changing it is described in [CAN Interface System Configuration](advos-yocto-system/can-bus-configuration.md) |
| Termination | See [Vehicle CAN bus](interfaces.md#vehicle-can-bus) |
...
| Speaking J1939 instead | A bidirectional bridge lets your existing J1939 stack talk to the controller unchanged — see [§3.3](#33-if-your-vehicle-speaks-j1939) |
...
### 3.1 What the controller sends you
...
| [DC_Control](vehicle-can-interfaces/can_v2.md#dc_control) | 0x602 | Commands to the BMS, including [Close_Contactors](vehicle-can-interfaces/can_v2.md#DC_Control-Close_Contactors) |
| [MCS_Extra_Information](vehicle-can-interfaces/can_v2.md#mcs_extra_information) | 0x606 | MCS protocol detail, for information |
...
| [EVCC_MEVC_Diagnostic_Status](vehicle-can-interfaces/can_v2.md#evcc_mevc_diagnostic_status) | 0x605 | Active faults and errors, plus charger status received over HLC — see [§10](#10-diagnostics) |

### 3.2 What you send the controller
...
| [DC_Status1](vehicle-can-interfaces/can_v2.md#dc_status1) | 0x612 | Always | Current/voltage requests and limits |
| [DC_Status2](vehicle-can-interfaces/can_v2.md#dc_status2) | 0x613 | Always | Battery and **inlet** voltage, contactor status, end-of-charge |
...
| [ADM_CS_EVCC_MEVC_Outputs](vehicle-can-interfaces/can_v2.md#adm_cs_evcc_mevc_outputs) | 0x617 | Only to drive the controller's outputs | See [§8](#8-digital-inputs-outputs-and-leds) |

Some of these can be provided **by the controller instead of by you** — see
[§6](#6-voltage-and-current-measurement).
...
### 3.3 If your vehicle speaks J1939
...
your existing J1939 stack talks to the controller unchanged. This is the common case on heavy-duty
vehicles, where J1939 is already the vehicle bus.
...
[§3.1](#31-what-the-controller-sends-you) and [§3.2](#32-what-you-send-the-controller) still
...
## 4. Charging protocol and session behaviour

**There is no protocol to select.** MCS sessions always run **ISO 15118-20**, over 10BASE-T1S.
DIN 70121 and ISO 15118-2 do not apply to MCS.

What *is* yours to get right is everything around the protocol: the two prerequisites without which
no session starts, what the vehicle declares to the charger, and how the session is allowed to
behave. That is the rest of this section.

### 4.1 Two prerequisites

| Prerequisite | Where | Why |
|---|---|---|
| Generic CAN interface **v2** | `[vehicle] type = Advantics_Generic_v2` ([type](configuration/generalities.md#type)) | ISO 15118-20 carries information — energy requests, dynamic limits, bidirectional signalling. |
...
<!-- !!! note "Plug & Charge is not available on ISO 15118-20"
    `enable_pnc` and the `pnc_contract_p12*` entries exist in `[ccs]`, but Plug & Charge is
    implemented for ISO 15118-2 only, which the MEVC does not run. MCS sessions authorise by EIM.
    See [TLS](configuration/tls.md#plug-charge). -->


### 4.2 How the session is allowed to behave

| Choice | Entry |
|---|---|
| Which -20 control mode to ask for | [`preferred_control_mode`](configuration/mcs.md#preferred_control_mode) — `Dynamic` (default) or `Scheduled` |
| Keep cycling power at 100 % SoC in a bidirectional session | [allow_bpt_at_full_soc](configuration/mcs.md#allow_bpt_at_full_soc) in `[vehicle]` — see [§13](#13-bidirectional-power-transfer-iso-15118-20) |

[`preferred_control_mode`](configuration/mcs.md#preferred_control_mode) represents ISO15118-20 control modes, the default is [Dynamic Mode](vehicle-can-interfaces/sequences_v2.md#power-transfer-dynamic-mode). The other option is the [Scheduled Mode](vehicle-can-interfaces/sequences_v2.md#power-transfer-scheduled-mode).

### 4.3 Holding the session off until your High Voltage system is ready

A vehicle might not have its HV system ready the instant the plug is in — HV reconnection, a BMS or other components waking from sleep, etc. Rather than have the charger start an insulation test into a system that is not
ready, the controller can hold the session at `Connected_With_Full_Info`, which is the last stage
before the powered ones.

| Choice | Entry |
|---|---|
| Always wait for the BMS before entering powered states. BMS comfirms ready state by setting [EV_Status.HV_Preparing_Hold_Off](vehicle-can-interfaces/can_v2.md#EV_Status-HV_Preparing_Hold_Off) | [`hold_off_until_bms_ready`](configuration/mcs.md#hold_off_until_bms_ready) in `[ccs]` — default `false` |
...
| How long the hold may last before the session aborts | [wait_hv_ready_timeout_ms](configuration/mcs.md#wait_hv_ready_timeout_ms) — default 40 s |

The two are different tools. The CAN signal is the normal one: raise it while you are not ready,
clear it when you are. [`hold_off_until_bms_ready`](configuration/mcs.md#hold_off_until_bms_ready) is the belt-and-braces version — with it set, the
controller waits at that stage on *every* session until the BMS starts sending CAN messages *and*
explicitly clears the signal, which is what you want if the controller can boot before your BMS and the rest of the vehicle does. Either way the wait is bounded by [`wait_hv_ready_timeout_ms`](configuration/mcs.md#wait_hv_ready_timeout_ms), and hitting that bound aborts the session.

## 5. DC contactor control
...
Independently of software, loss of **CE** triggers a hardware-level disconnect to meet the timing
required by the standards — see
...
The MCS inlet lock is a DC motor driving a pin, energised in one polarity to lock and the other to
...
## 8. Digital inputs, outputs and LEDs
...
The MCS inlet's PT1000 sensors wire straight to the controller, which terminates the charge if
limits are exceeded — damaged inlets and cables get dangerously hot at MCS current levels.
...
| [`max_soc`](configuration/generalities.md#max_soc) reached | Controller triggers a normal stop |
| `Emergency_Stop` digital input | Emergency path, see [§5.2](#52-emergency-opening) |
...
| Loss of CE | Hardware disconnect, independent of software |
...
- Keep the controller powered while it sleeps or it loses its state and cold-boots.

Details, wake-up events and power figures: [Sleep functions](vehicle-features/evcc_sleep.md).

## 13. Bidirectional power transfer (ISO 15118-20)

MCS sessions can be bidirectional. Set
...
<!-- ## 14. Bringing it up, in order

1. Read [Interfaces](interfaces.md) and wire power, the inlet harness and the CAN bus. Check bus
   termination.
2. Edit the config file `/srv/config.cfg` using the web interface or directly via SSH — at minimum `max_current`, `max_voltage`, `target_voltage`,
   `min_voltage`, `energy_capacity`.
3. Run sessions with [charger simulator](https://documentation.advantics.fr/adm-cs-spcc/charger-features/charger-simulation.html) at low, safe voltage and current. This proves the inlet, lock, contactors and protocol stack.
...
6. Work through the [Deployment Checklist](advos-yocto-system/must-do-before-deploy.md) before
   shipping. -->

## 14. Frequently hit questions
...
**The session never starts, or drops at random.** Check the CE and ID lines first, and consider the
median filter and debouncer in [§2](#2-the-mcs-inlet-interface).

**The cable stays locked and I cannot remove it.** That is deliberate when the controller cannot
prove the situation is safe. See [§7](#7-inlet-lock); do not force it, and never use the manual
release during operation.

**Do I have to implement all the messages?** No. `EV_Information`, `DC_Status1` and `DC_Status2` are
the working minimum, plus the energy-request messages for ISO 15118-20; a CAN sensor removes some of
them entirely.

**Can the vehicle keep control of its own contactors?** Yes — that is the default. See
[§5](#5-dc-contactor-control).
