...
Up to 800V DC bus and 500V output operation is possible. For more details on the specifications, please check the [ADM-PC-LL25 specifications](5_Mechanical_and_Electrical_Technical_Overview.md).

![LLC topology](assets/LLC_topology-LLC_topology.svg){ width="70%" }
...
![LLC absolute range](assets/LLC_max_absolute_range.JPG){ width="50%" }
...
!!! warning
    Only the last LLC hardware revision (rev. 4), is able to reach 70 Amps as max current. For previous hardware revisions, maximum reachable current is 60 Amps. Heatsink and cooling restrictions must still be taken into account to derate this value.

!!! tip
    **Always ask ADVANTICS** if you are not sure wether the module can operate in a specific operating point or in certain scenarios.
...
![LLC single ratio](assets/LLC_single_ratio_range.JPG){ width="40%" }
...
To operate the LLC, use the 'Voltage control' mode. For more information, please refer to the [Control Modes](#control-modes) subsection.
...
![LLC absolute range](assets/LLC_combo_range.JPG){ width="40%" }
<figcaption style="text-align: center">ADM-PC-LL25 operating range when used with the Advantics combo</figcaption>

!!! note
    **The BP25 or the UP25 must have the same stack address as the LLC to work** properly.

!!! warning
    When used with the UP25 module, the maximum output current should NOT be above 52 Amps. This is due to UP25 limitations.

To operate the LLC with the Advantics combo, use the 'PPFC Voltage control' mode. For more information, please refer to the [Control Modes](#control-modes) subsection.
...
!!! note
    **If a resistive load is present at the output, then, the minimum resistance of the load must be 4 Ohms**. Reachable operating range above still applies.

!!! warning
    The module is not able to generate an open circuit voltage. Thus, the module has to be enabled always under a load (either a battery or a resistive load), or it has to be precharged at the output with the 'precharge' operation mode.
...
- If the user sends a "bleed" command over CAN. Refer to the [CAN messages](can_bus_interface.md#LLC_Fault_Control) for more information on the message to send.
...
To start operating the device, please refer to the [Quick Start](6_Setting_up_ADM_PC_LL25.md#quick-start) section. However, it can be summarized in:
...
!!! tip
    Refer to [Application examples](4_Applications.md) to see typical circuit applications and connection diagrams.
...
![basic connection](assets/LLC_basic_connection-LLC_basic_connection.svg){ width="60%" }
...
![basic connection](assets/app_3phase_charger.svg){ width="80%" }
