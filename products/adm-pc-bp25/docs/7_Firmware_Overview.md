...
![AFE topology](assets/afe_efficiency.svg){ width="60%" }
...
For specific information about the message, please check the [CAN database](can_bus_overview.md) section.
...
![power envelope va08](assets/power_envelope_va08.svg){ width="60%" }
...
To start operating the device, please refer to the [Quick Start](6_Setting_up_ADM_PC_BP25.md#quick-start) section. However, it can be summarized in:
...
!!! warning
    Do not use this mode unless it is required by Advantics for test purposes
...
!!! note
    The frequency setpoint must be set BEFORE the Neutral mode is enabled.

For more information about possible applications, please refer to the [Application Examples](4_Applications.md) section.

![power envelope va01](assets/AFE_neutral-AFE_neutral.svg){ width="50%" }
...
![power envelope va01](assets/AFE_buck-AFE_buck.svg){ width="50%" }
...
![power envelope va08](assets/AFE_boost-AFE_boost.svg){ width="70%" }
...
![power envelope va08](assets/AFE_boost_neutral-AFE_boost_neutral.svg){ width="70%" }
...
![power envelope va08](assets/AFE_rectifier_1ph-AFE_rectifier_1ph.svg){ width="70%" }
...
![power envelope va08](assets/AFE_rectifier_1ph_buck-AFE_rectifier_1ph_buck.svg){ width="70%" }
...
![power envelope va08](assets/AFE_rectifier_3ph-AFE_rectifier_3ph.svg){ width="70%" }
...
!!! note
    The frequency setpoint must be set BEFORE the Inverter 1-phase mode is enabled.

![power envelope va08](assets/AFE_inverter_1-ph-AFE_inverter_1ph.svg){ width="60%" }
<figcaption style="text-align: center">Simple connection of ADM-PC-BP25 in boost mode with precharge</figcaption>
...
![power envelope va08](assets/AFE_inverter_1-ph_boost-AFE_inverter_1-ph_boost.svg){ width="70%" }
...
!!! note
    The frequency and phase setpoints must be set BEFORE the Inverter 1-phase Sync. mode is enabled.


![power envelope va08](assets/AFE_inverter_1-ph_sync-AFE_inverter_1-ph_sync.svg){ width="50%" }
...
!!! note
    The frequency setpoint must be set BEFORE the Inverter 3-phase mode is enabled.

![power envelope va08](assets/AFE_inverter_3ph-AFE_inverter_3ph.svg){ width="50%" }
...
![power envelope va08](assets/AFE_inverter_1-ph_distributed.svg){ width="50%" }
...
Please, refer to the [CAN database](can_bus_overview.md) for more information about the messages to set/retrieve the group ID.
...
!!! warning
    Setpoints and control mode for devices working in parallel must be the same.

!!! warning
    Do not change the Group ID while the modules are enabled.

!!! note
    Even though modules are connected in parallel, one module might be more loaded than others (i.e. perfect sharing of the load is not guaranteed)
...
![power envelope va08](assets/parallel_boost_buck-parallel_boost_buck.svg){ width="90%" }
...
![power envelope va08](assets/diagrams-parallel_boost_buck_v2.svg){ width="90%" }
...
!!! warning
    These overload conditions must always be short in time, and steady state current should always be equal or below the nominal current of the converter (**30/33 Amps RMS per phase** for VA01/VA08 respectively)
...
!!! note
    For Inverter 3-phase mode, if one phase is more loaded than the other (unbalanced case), the voltage will drop equally in all phases based on the most loaded phase.

The V-I (voltage-current) curve of the converter then looks like the following picture:

![V-I curve](assets/inverter_curve.svg){ width="50%" }
...
!!! tip
    We recommend to start by setting Iref = 30 Amps (90 Amps in the case of Inverter 1-phase + SYNC mode), then checking the behaviour and adjust this value as needed.
...
!!! tip
    Modules should also be able to be paralleled with diesel generators, but experience with this is extremely limited, also due to the big differences between diesel generator specifications (both steady state and transient performance)
...
!!! note
    When no load is connected, there will be some circulating current, mostly reactive. This is unavoidable, but this small reactive current does not take much of the current budget, as it adds in quadrature to active current. When loads are connected, the AFEs will tend to share the load more or less equally.
...
!!! warning
    **Important:** the modification of these parameters has serious implications on the behavior and stability of the module. If you need assistance, ask Advantics.
