...
| Release 4.2 | 2025-12-23 | evse-controller 3.4.0:<br/>- Support for Pause/Resume functionality according to ISO15118-2 and ISO15118-20<br/>- Allow restarting a new charge session when B-C-B toggle is detected (CCS)<br/>- Add CCS and MCS extra info messages reporting CP, duty cycle / CE, ID<br/>- Change default current ramp up/down rates<br/>- Fix EV ID sending over CAN bus interface<br/>- Boost-Buck interface: Force boost to always have a 200V offset over buck voltage<br/>- Boost-Buck interface: Perform precharge using all the stacks<br/>- Boost-Buck interface: Turn off bucks in standby<br/><br/>ccs-secc 2.4.0:<br/>- Support for Pause/Resume functionality according to ISO15118-2 and ISO15118-20<br/>- MCS: Add configurable software filtering on CE and ID lines<br/><br/>advantics-csm 1.6.4:<br/>- Add the possibility to restart CSM when submitting config from the UI<br/> | [Release 4.2](https://drive.google.com/uc?export=download&id=1hH_n4QDW-mkYWb9ZWNQoTAmSQ2VjW_E-) | Update instructions here ([Full release update](charge-controllers/sys3_update.md#full-release-update)). |
| Release 4.3 | 2026-03-13 | evse-controller 3.6.0:<br/>- Inputs messages are now suppressed until hardware I/O initialization is fully complete (startup safety)<br/>- Frontend is notified when a session ends while the pistol is still plugged in, so the CP oscillator can be disabled promptly<br/>- Temperature median filter window is now configurable (SECC/SPCC)<br/>- Fixed encoding of negative target_voltage values sent by the car<br/><br/>ccs-secc 2.5.0:<br/>- Disabled the CP oscillator at end of session<br/><br/>advantics-csm 1.7.0:<br/>- Fix logic while exporting logs, server would respond with error code and nothing attached<br/>- Add the ability to generate sample config for every controller<br/>- Fix dynamic voltage meter bar in monitoring and use kW for power metering instead of W<br/>- Remove unused items from navigation side bar<br/>- Minor improvements | [Release 4.3]() | Update instructions here ([Full release update](charge-controllers/sys3_update.md#full-release-update)). |
| Release 4.4 | 2026-04-10 | evse-controller 3.6.1:<br/>- Fixed onefile mode folders not getting cleared properly on power cycle<br/><br/>ccs-secc 2.6.0:<br/>- Fixed onefile mode folders not getting cleared properly on power cycle<br/><br/>advantics-csm 1.7.1:<br/>- Fix temperature config section<br/>- Bring back controller type to header<br/>- Add CSM version on footer<br/>- Migrate from remix to react router 7 | [Release 4.4](https://pub-ec884f5e1c6b4942867b3ac199d79823.r2.dev/secc/secc-release-4.4.zip) | Update instructions here ([Full release update](charge-controllers/sys3_update.md#full-release-update)). |
