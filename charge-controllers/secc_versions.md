...
| Release 4.3 | 2026-03-13 | evse-controller 3.6.0:<br/>- Inputs messages are now suppressed until hardware I/O initialization is fully complete (startup safety)<br/>- Add CAN message ADM_CS_SPCC_Outputs to control the ADM-CS-SPCC outputs<br/>- Frontend is notified when a session ends while the pistol is still plugged in, so the CP oscillator can be disabled promptly<br/>- Temperature median filter window is now configurable (SECC/SPCC)<br/>- Fixed encoding of negative target_voltage values sent by the car<br/><br/>ccs-secc 2.5.0:<br/>- Ignore spurious CE change to B0 when Ss3 is still closed (erroneous measurement during B→C transition)<br/>- Reset charger info and charger request-stop flag at the start of each new session<br/>- Disabled the CP oscillator at end of session<br/><br/>advantics-csm 1.7.0:<br/>- Fix logic while exporting logs, server would respond with error code and nothing attached<br/>- Add the ability to generate sample config for every controller<br/>- Fix dynamic voltage meter bar in monitoring and use kW for power metering instead of W<br/>- Remove unused items from navigation side bar<br/>- Minor improvements | [Release 4.3](https://pub-ec884f5e1c6b4942867b3ac199d79823.r2.dev/secc/secc-release-4.3.zip) | Update instructions here ([Full release update](charge-controllers/sys3_update.md#full-release-update)). |
